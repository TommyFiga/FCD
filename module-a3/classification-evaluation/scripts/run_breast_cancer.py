...
from src.data_loader import load_dataset, preprocess_breast_cancer_dataset
from src.evaluation import evaluate_classifiers, generate_confusion_matrix, generate_metrics


def run_breast_cancer():
    filepath = DATA_DIR / 'breast-cancer.csv'
    output = OUTPUT_DIR / 'breast-cancer' 

    print('Loading dataset...')
    df = load_dataset(filepath)

    print('Preprocessing dataset...')
    X, y = preprocess_breast_cancer_dataset(df)
